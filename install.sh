...
#   5. 解压到 ~/.openharness-sql/versions/<ver>/
#   6. 原子切换 ~/.openharness-sql/current 符号链接
#   7. 写入 ~/.openharness-sql/bin/ohsql shim
...
cat > "$SHIM" <<'SHIM_EOF'
#!/usr/bin/env bash
exec node "$HOME/.openharness-sql/current/dist/cli.js" "$@"
SHIM_EOF
chmod +x "$SHIM"
